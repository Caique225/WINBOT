	...
	client.on('chat-update', async (mek) => {
		try {
			...
			switch(command) {
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`http://imgur.com/a/ng8nEE1`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Winchester\n*YOUTUBE:* não tem\n*WPP:* wa.me/+5538999221121\n*INSTA:* caiquewinchester_ofc\n\n\n*Digite *.bot* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'clearall':
					...
					reply('Sucesso deleta todo o chat :)')
					break
				case 'bc':
				...
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ responde uma figurinha lek ❌')
					reply(mess.wait)
				...
				case 'clone':
					...
					try {
						...
						mentions(`Foto do perfil atualizada com sucesso com a foto do perfil @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('Om falha')
					}
					break
